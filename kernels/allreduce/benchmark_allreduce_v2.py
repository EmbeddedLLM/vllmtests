...
class BenchmarkRunner:
    ...
    def init_implementations(self):
        ...
        try:
            ...
            # Use larger buffer size to handle bigger tensors
            max_size = max(TEST_SIZES)  # Double the max test size for safety
            self.implementations['aiter'] = AITERCustomAllreduce(
                group=self.gloo_group,
                device=self.device,
                max_size=max_size
            )
            print(f"Rank {self.rank}: AITER custom all-reduce initialized with max_size={max_size}")
        ...
        try:
            ...
            print(f"Rank {self.rank}: vLLM custom all-reduce initialized with max_size={max_size}")
        except Exception as e:
    ...
    def benchmark_aiter(self, tensor: torch.Tensor) -> Dict[str, float]:
        ...
        try:
            # Register buffer for this tensor size
            ...
            print(f"Rank {self.rank}: AITER registered failed for size {tensor.numel() * tensor.element_size()}: {e}")
        ...
        try:
            ...
            print(f"Rank {self.rank}: AITER unregistered failed for size {tensor.numel() * tensor.element_size()}: {e}")
    ...
    def benchmark_vllm_custom(self, tensor: torch.Tensor) -> float:
        ...
        try:
            ...
            print(f"Rank {self.rank}: vLLM custom failed for size {tensor.numel() * tensor.element_size()}: {e}")
            return -1
    ...
    def benchmark_vllm_quick(self, tensor: torch.Tensor) -> Dict[str, float]:
        ...
        for regime in QuickReduceRegime:
            try:
                ...
                print(f"Rank {self.rank}: vLLM quick {regime.name} failed for size {tensor.numel() * tensor.element_size()}: {e}")
    ...
    def run_benchmark(self):
        ...
        for size_bytes in TEST_SIZES:
            ...
            try:
                ...
                print(f"Rank {self.rank}: NCCL failed for size {size_bytes}: {e}")
                self.results[size_key]['nccl'] = -1

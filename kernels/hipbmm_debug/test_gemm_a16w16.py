...
@perftest(num_iters=TEST_NUM_ITERS)
def aiter_hip_bpreshuffle(inp, weights, scaleA, scaleB, dtype, bias=None):
    if scaleB is not None:
        scaleB = scaleB.t()
    return hipb_mm(
        inp,
...
@benchmark()
def test_gemm(dtype, m, n, k, bias=False, otype=None, scaleA=None, scaleB=None, fused_bias=False):
    dim = (m, n, k)
    ...
    if (
        ...
        if fused_bias:
            c, avg_c = aiter_hip_bpreshuffle(x, weight_bpreshuffle, None, None, otype, bias)
        else:
    ...
    err_tgemm = checkAllclose(a, b, msg=msg_b, atol=5e-3, rtol=5e-3)
    err_hipb = checkAllclose(a, c, msg=msg_c, atol=5e-3, rtol=5e-3) if c is not None else None

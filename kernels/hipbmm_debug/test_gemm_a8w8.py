...
@perftest(num_iters=TEST_NUM_ITERS)
def run_aiter_hip_bpreshuffle(inp, weights, scaleA, scaleB, dtype, bias=None):
...
@benchmark()
def test_gemm(dtype, m, n, k, quantDtype=dtypes.i8, fused_bias=False):
    dim = (m, n, k)
    ...
    if quantDtype == dtypes.fp8 and get_gfx() == "gfx942" and dtype == dtypes.bf16:
        ...
        init_hipblas()
        if fused_bias:
            e, avg_e = run_aiter_hip_bpreshuffle(x, weightshuffle, x_scale, w_scale, dtype, bias)
        else:
...
def test_normal_gemm_a8w8_pertoken_quant(l_dtype, l_quantDtype, l_mnk, fused_bias=False):
    df = []
    for dtype in l_dtype:
        for quantDtype in l_quantDtype:
            for m, n, k in l_mnk:
                ret = test_gemm(dtype, m, n, k, quantDtype, fused_bias)
                df.append(ret)
    df = pd.DataFrame(df)
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="""Fuse bias addition with matmul.
    e.g.: -fb or --fused_bias""",
)
...
test_normal_gemm_a8w8_pertoken_quant(l_dtype, l_quantDtype, l_mnk_nm, args.fused_bias)
